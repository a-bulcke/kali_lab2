...
# On garde le nom de réseau tel que défini dans le lab1
# On ne sera pas obligé de supprimer ce dernier (mais uniquement de le stopper)
NOM_RESEAU_CLIENTS="bridge_interne_lab"
NOM_RESEAU_SERVEURS="bridge_serveurs_lab"
# La configuration du DNS doit être modifiée si les valeurs ci-dessous sont modifiées
# Modifier la variable IP_RESEAU_CLIENTS ou IP_RESEAU_SERVEURS implique la suppression de tous les conteneurs et volumes associés
IP_RESEAU_CLIENTS="192.168.56.0/24"
IP_RESEAU_SERVEURS="172.16.10.0/24"
IP_SERVEUR="172.16.10.10"
IP_CLIENT="192.168.56.11"
IP_KALI="192.168.56.12"
IP_ROUTEUR_CLIENTS="192.168.56.254"
IP_ROUTEUR_SERVEURS="172.16.10.254"
IP_METASPLOITABLE="172.16.10.5"
...
VOL_SERVEUR="home_serveur_lab2"
VOL_CLIENT="home_client_lab2"
VOL_KALI="home_kali_lab2"
VOL_ROUTEUR="home_routeur_lab2"
...
IMAGE_CLIENT="$ID_DOCKER/clientdebian12:lab2"
IMAGE_KALI="$ID_DOCKER/kalirolling:lab2"
IMAGE_ROUTEUR="$ID_DOCKER/routeurdebian12:lab2"
IMAGE_BASE=$ID_DOCKER/basedebian12:1.0
IMAGE_METASPLOITABLE=tleemcjr/metasploitable2

# Nom du conteneur
SERVEUR="serveur-lab2"
CLIENT="client-lab2"
KALI="kali-lab2"
ROUTEUR="routeur-lab2"
METASPLOITABLE="metasploitable-lab2"
...
while getopts "cldsri:h" option; do
    ...
    # -i Crée une image personnalisée (type de l'image en majuscule)  -i ROUTEUR|SERVEUR|CLIENT|KALI|BASE."
...
# Fonctions qui modifient les passerelles par défaut
# Qui doit être (sauf pour le routeur), l'adresse IP du routeur

MODIF_PASSERELLE_CLIENTS() {
    NOM_CONTENEUR=$1
    docker exec --privileged "$NOM_CONTENEUR" ip route del default
    docker exec --privileged "$NOM_CONTENEUR" ip route add default via "$IP_ROUTEUR_CLIENTS"
}

MODIF_PASSERELLE_SERVEURS() {
    NOM_CONTENEUR=$1
    docker exec --privileged "$NOM_CONTENEUR" ip route del default
    docker exec --privileged "$NOM_CONTENEUR" ip route add default via "$IP_ROUTEUR_SERVEURS"
}
...
MODIF_PASSERELLE_CONTENEURS() {
    ...
    sleep 5
    echo -e "\nModification des passerelles du serveur, de Kali, du client et du serveur metasploitable."
    MODIF_PASSERELLE_SERVEURS "$SERVEUR"
    MODIF_PASSERELLE_CLIENTS "$CLIENT"
    MODIF_PASSERELLE_CLIENTS "$KALI"
    MODIF_PASSERELLE_SERVEURS "$METASPLOITABLE"
}
...
CONFIGURATION_ROUTEUR() {
    ...
    if ! (docker network inspect $NOM_RESEAU_CLIENTS | grep $ROUTEUR >/dev/null); then
        docker network connect $NOM_RESEAU_CLIENTS --ip "$IP_ROUTEUR_CLIENTS" "$ROUTEUR"
        echo -e "Ajout de l'interface connectée au réseau interne... Fait"
    else
        echo -e "Interface connectée au réseau interne... Présente"
    fi

    # Ajout de la carte connectée au réseau des serveurs - eth2
    if ! (docker network inspect $NOM_RESEAU_SERVEURS | grep $ROUTEUR >/dev/null); then
        docker network connect $NOM_RESEAU_SERVEURS --ip "$IP_ROUTEUR_SERVEURS" "$ROUTEUR"
        echo -e "Ajout de l'interface connectée au réseau interne... Fait"
    else
        echo -e "Interface connectée au réseau interne... Présente"
    fi
    ...
    if ! (docker exec --privileged "$ROUTEUR" iptables -t nat -L | grep dpt:"$SSH_PORT_METASPLOITABLE" >/dev/null); then
        docker exec --privileged "$ROUTEUR" iptables -t nat -A PREROUTING -p tcp --dport "$SSH_PORT_METASPLOITABLE" -j DNAT --to-destination "$IP_METASPLOITABLE":22
        echo -e "Ajout de la redirection SSH du serveur Metasploitable... Fait"
    else
        echo -e "Redirection SSH du serveur Metasploitable... Présente"
    fi
...
if [ -n "$create" ]; then
    ...
    echo -e "\nCréation du réseau $IP_RESEAU_CLIENTS pour le lab"
    if (docker network ls | grep bridge_interne_lab >/dev/null); then
        NET_ACTUEL=$(docker network inspect --format='{{range .IPAM.Config}}{{.Subnet}}{{end}}' bridge_interne_lab)
        if [ "$IP_RESEAU_CLIENTS" ] && [ "$NET_ACTUEL" != "$IP_RESEAU_CLIENTS" ]; then
            # shellcheck disable=SC2046
            docker rm -f $(docker ps -aq)
            # shellcheck disable=SC2046
            docker volume rm $(docker volume ls -qf dangling=true)
            docker network rm bridge_interne_lab
            docker network create \
                --driver=bridge \
                --subnet="$IP_RESEAU_CLIENTS" \
                "$NOM_RESEAU_CLIENTS"
        else
            echo -e "Réseau $IP_RESEAU_CLIENTS pour le lab... Déjà créé."
        fi
    else
        docker network create \
            --driver=bridge \
    ...
    # Création du réseau des serveurs
    echo -e "\nCréation du réseau $IP_RESEAU_SERVEURS pour le lab"
    if (docker network ls | grep bridge_serveurs_lab >/dev/null); then
        NET_ACTUEL=$(docker network inspect --format='{{range .IPAM.Config}}{{.Subnet}}{{end}}' bridge_serveurs_lab)
        if [ "$IP_RESEAU_SERVEURS" ] && [ "$NET_ACTUEL" != "$IP_RESEAU_SERVEURS" ]; then
            # shellcheck disable=SC2046
            docker rm -f $(docker ps -aq)
            # shellcheck disable=SC2046
            docker volume rm $(docker volume ls -qf dangling=true)
            docker network rm bridge_serveurs_lab
            docker network create \
                --driver=bridge \
                --subnet="$IP_RESEAU_SERVEURS" \
                "$NOM_RESEAU_SERVEURS"
        else
            echo -e "Réseau $IP_RESEAU_SERVEURS pour le lab... Déjà créé."
        fi
    else
        docker network create \
            --driver=bridge \
    ...
    for conteneur in "$ROUTEUR" "$KALI" "$SERVEUR" "$CLIENT" "$METASPLOITABLE"; do
    ...
    if (docker run --name "$ROUTEUR" \
        ...
        -p "$SSH_PORT_METASPLOITABLE":"$SSH_PORT_METASPLOITABLE" \
    ...
    echo -e "\nLancement du serveur metasploitable"
    # Lancement du serveur
    if (docker run --name "$METASPLOITABLE" \
        --network "$NOM_RESEAU_SERVEURS" \
        --hostname "$HOST_METASPLOITABLE" \
        --ip "$IP_METASPLOITABLE" \
        --dns "$IP_SERVEUR" \
        --dns-search "$DOMAINE" \
        -t \
        -d \
        -v "$VOL_METASPLOITABLE":/home/msfadmin \
        "$IMAGE_METASPLOITABLE"); then
        echo -e "conteneur $METASPLOITABLE.. Créé"

    else
        echo -e "conteneur $METASPLOITABLE... Echec"
        exit 1
    fi
...
if [ -n "$delete" ]; then
    ...
    echo -e "\nSuppression du réseau $IP_RESEAU_CLIENTS"
    if (docker network ls | grep bridge_interne_lab >/dev/null); then
        docker network rm bridge_interne_lab
    else
        echo -e "Réseau $IP_RESEAU_CLIENTS... Déjà supprimé."
    fi

    echo -e "\nSuppression du réseau $IP_RESEAU_SERVEURS"
    if (docker network ls | grep bridge_serveurs_lab >/dev/null); then
        docker network rm bridge_serveurs_lab
    else
        echo -e "Réseau $IP_RESEAU_SERVEURS... Déjà supprimé."
    fi
